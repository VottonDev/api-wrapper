...
function buildWrapperFn(root, parseResult, method, requestModule,
                        requestOptions, shouldParseJson) {
...
module.exports.create = function(config) {
  ...
  const requestModule =
      requestDefaults ? request.defaults(requestDefaults) : request;
  const wrapper = {};
  ...
  getMethodIterator(config, function(method, key, value) {
    ...
    wrapper[key] = buildWrapperFn(root, parseResult, method, requestModule,
                                  requestOptions, shouldParseJson);
  });
